...
const SearchSection: React.FC<SearchSectionProps> = ({ students, onResult, isDarkMode = false }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (filteredResults.length > 0) {
        onResult(filteredResults[0]); // إرجاع أول نتيجة
      } else {
        onResult(null);
      }
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12 animate-slideInDown">
            <div className="flex justify-center items-center gap-3 mb-4">
              <Trophy className="w-12 h-12 text-blue-600 animate-bounce-slow" />
